...
public class PrometheusPowerTest {
    ...
    @Test
    @DisplayName("Check Prometheus Power" )
    void prometheusPowerTest() throws NotExecutedException {
        controller.getGame().getCurrentPlayer().setGod(GodType.PROMETHEUS);
        controller.getGame().getCurrentPlayer().getGod().activateGodPower(controller.getGame().getCurrentPlayer().getAllWorkers().get(0));
        assertTrue(controller.getGame().getCurrentPlayer().getAllWorkers().get(0).computeAvailableSpaces().contains(controller.getGame().getWorld().getSpaces(0,0)));
        assertTrue(controller.getGame().getCurrentPlayer().getAllWorkers().get(0).computeBuildableSpaces().contains(controller.getGame().getWorld().getSpaces(1,0)));
        controller.build(controller.getGame().getCurrentPlayer().getAllWorkers().get(0),controller.getGame().getWorld().getSpaces(1,0));
        ...
        assertTrue(controller.getGame().getCurrentPlayer().getAllWorkers().get(0).computeAvailableSpaces().contains(controller.getGame().getWorld().getSpaces(0,2)));
        assertTrue(!controller.getGame().getCurrentPlayer().getAllWorkers().get(0).computeAvailableSpaces().contains(controller.getGame().getWorld().getSpaces(0,0)));
    ...
    void spaceSetup(){
        ...
        for(int i = 0; i < 1; i++) world.getSpaces(0, 0).addLevel(); //[0][0] level 1
