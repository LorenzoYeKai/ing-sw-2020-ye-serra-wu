...
public class PanPower {
    ...
    void init(){
        ...
        spaceSetup();
        Space firstWorkerPosition = controller.getGame().getWorld().getSpaces(0, 0);
    ...
    @Test
    @DisplayName("Check Pan Power" )
    void panPowerTest() throws NotExecutedException {
        controller.getGame().getCurrentPlayer().setGod(GodType.PAN);
        ...
        assertTrue(controller.getGame().getListOfPlayers().get(1).isDefeated());
    ...
    void spaceSetup(){
        ...
        for(int i = 0; i < 2; i++) world.getSpaces(0, 0).addLevel(); //[2][2] level 2
