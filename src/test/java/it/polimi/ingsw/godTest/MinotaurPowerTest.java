...
public class MinotaurPowerTest {
    ...
    void init(){
        ...
        Space thirdWorkerPosition = controller.getGame().getWorld().getSpaces(2, 2);
    ...
    @Test
    @DisplayName("Check Minotaur Power" )
    void minotaurPowerTest() throws NotExecutedException {
        controller.getGame().getCurrentPlayer().setGod(GodType.MINOTAUR);
        assertTrue(!controller.getGame().getCurrentPlayer().getAllWorkers().get(0).computeBuildableSpaces().contains(controller.getGame().getWorld().getSpaces(2,2)));
        controller.getGame().getCurrentPlayer().getGod().activateGodPower(controller.getGame().getCurrentPlayer().getAllWorkers().get(0));
        assertTrue(controller.getGame().getCurrentPlayer().getAllWorkers().get(0).computeAvailableSpaces().contains(controller.getGame().getWorld().getSpaces(2,2)));
        controller.move(controller.getGame().getCurrentPlayer().getAllWorkers().get(0),controller.getGame().getWorld().getSpaces(2,2));
        assertTrue(controller.getGame().getWorld().getSpaces(2,2).isOccupiedByWorker());
        assertTrue(controller.getGame().getWorld().getSpaces(3,3).isOccupiedByWorker());
